...
- **Never use em dashes**, in app copy, comments, docs, commit messages, or
  replies. Cole asked for all 365 of them to be stripped and does not want them
  back. Use the punctuation the sentence actually wants (a colon when the second
  half explains the first, a period when both stand alone, commas for an aside);
  swapping in a hyphen reads worse than the em dash did.
